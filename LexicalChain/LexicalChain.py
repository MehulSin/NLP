...
is_noun = lambda pos: True if (pos == 'NN' or pos == 'NNP' or pos == 'NNS' or pos == 'NNPS') else False
...
for chain in lexical_chains:
    list1.append(", ".join(str(word + "(" + str(dictionary[word]) + ")") for word in chain.getWords()))
cnt = 0
summary = [listitems for listitems in list1 if len(listitems.split(','))> 1]
'''for listitems in list1:
    
      if len(list2) > 1 :
        cnt = cnt +1
        print("Chain.{}".format(cnt),listitems)
        summary.append(listitems)'''
print('\n')
print("THE ENTIRE LEXICAL CHAIN LIST :")
cnt = 0
for listitems in list1:
    cnt = cnt +1
    print("Chain.{}".format(cnt),listitems)
cnt = 0
print('\n\n\n')
print("THE Text summary report :")
for listitems in summary:
    cnt = cnt +1
    print("Chain.{}".format(cnt),listitems)
